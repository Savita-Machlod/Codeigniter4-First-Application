...
class Employees extends BaseController
{
	public function index()
	{
		...
		if ($this->request->getMethod() == 'post') {
			...
			$errors = [
				'password' => [
					'validateEmployee' => 'Email or Password don\'t match'
				]
			];
			...
			if (! $this->validate($rules, $errors)) {
				...
				$employee = $model->where('email', $this->request->getVar('email'))
											->first();
	...
	private function setEmployeeSession($employee){
		$data = [
			'id' => $employee['id'],
	...
	public function profile(){
		...
		$data['employee'] = $model->where('id', session()->get('id'))->first();
		echo view('templates/header', $data);
		echo view('profile');
		echo view('templates/footer');
		//return view('profile');
